...
func TestCacheBasic(t *testing.T) {
	...
	t.Run("删除操作", func(t *testing.T) {
		...
		// 修复后：用 deleted 字段标记删除，expireAt 保留原始值（不再清零）
		if !node.deleted {
			t.Fatal("删除后节点 deleted 应为 true")
		}
		if node.expireAt != 100 {
			t.Fatalf("删除后 expireAt 应保留原始值100，实际为%d", node.expireAt)
		}
		if expireAt != 100 {
			t.Fatalf("删除应返回原始expireAt(100)，实际为%d", expireAt)
		}

		// 验证删除后无法获取（del 标记了 deleted=true，再次 del 应返回 status=0）
		node, status = c.get("key1")
		if status != 1 {
			t.Fatal("获取已删除项失败，但键仍应存在于哈希表中")
		}
		// 修复后：deleted=true 表示已删除，expireAt 保留原值
		if !node.deleted {
			t.Fatal("已删除项的 deleted 应为 true")
		}
...
func TestLRU2StoreHitRatio(t *testing.T) {
	...
	// 修复后：level1 满时淘汰的项晋升到 level2 而不是直接丢弃
	// 4桶×10容量=40个 level1 槽，50个 key 中约 40 个留在 level1，10 个晋升到 level2
	// 查询 key0-key99（50个存在，50个不存在），命中率预期约 0.45-0.55
