...
func (g *Group) syncToPeers(ctx context.Context, op string, key string, value []byte) {
	...
	// 通过 gRPC metadata 传递 from-peer 标记
	// 不能用 context.Value：它只在同一进程内有效，跨进程 gRPC 调用时会丢失
	// gRPC metadata 会随请求头一起发送到远端节点
	syncCtx := metadata.NewOutgoingContext(
		context.Background(),
		metadata.Pairs("from-peer", "true"),
	)
